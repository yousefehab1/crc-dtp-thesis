...
Referenced from Section 3.3 (Figure 7). Pairwise Mann-Whitney comparisons of each DTP score (up, down, composite) between subtype levels within CMS and PDS, in GSE39582 and TCGA-COAD. Columns: Dataset, Subtype_Axis (CMS or PDS), Score, the two groups compared (Group_A, Group_B) with their sizes and medians, the rank-biserial effect size, raw and Benjamini-Hochberg corrected p-values (per axis and global), and testability and significance flags. Downloadable supplementary file: [AppendixTableA7_subtype_pairwise.csv](tables/AppendixTableA7_subtype_pairwise.csv). Source: run CRC_DTP_20260806_1651, crc_survival module.
